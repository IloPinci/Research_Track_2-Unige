// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from bme_gazebo_sensors:action/MoveAction.idl
// generated code does not contain a copyright notice
#include "bme_gazebo_sensors/action/detail/move_action__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
bme_gazebo_sensors__action__MoveAction_Goal__init(bme_gazebo_sensors__action__MoveAction_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // target_coordinate
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_Goal__fini(bme_gazebo_sensors__action__MoveAction_Goal * msg)
{
  if (!msg) {
    return;
  }
  // target_coordinate
}

bool
bme_gazebo_sensors__action__MoveAction_Goal__are_equal(const bme_gazebo_sensors__action__MoveAction_Goal * lhs, const bme_gazebo_sensors__action__MoveAction_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // target_coordinate
  if (lhs->target_coordinate != rhs->target_coordinate) {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_Goal__copy(
  const bme_gazebo_sensors__action__MoveAction_Goal * input,
  bme_gazebo_sensors__action__MoveAction_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // target_coordinate
  output->target_coordinate = input->target_coordinate;
  return true;
}

bme_gazebo_sensors__action__MoveAction_Goal *
bme_gazebo_sensors__action__MoveAction_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Goal * msg = (bme_gazebo_sensors__action__MoveAction_Goal *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_Goal));
  bool success = bme_gazebo_sensors__action__MoveAction_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_Goal__destroy(bme_gazebo_sensors__action__MoveAction_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_Goal__Sequence__init(bme_gazebo_sensors__action__MoveAction_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Goal * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_Goal *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_Goal__Sequence__fini(bme_gazebo_sensors__action__MoveAction_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_Goal__Sequence *
bme_gazebo_sensors__action__MoveAction_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Goal__Sequence * array = (bme_gazebo_sensors__action__MoveAction_Goal__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_Goal__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_Goal__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_Goal__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_Goal__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_Goal__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_Goal * data =
      (bme_gazebo_sensors__action__MoveAction_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
bme_gazebo_sensors__action__MoveAction_Result__init(bme_gazebo_sensors__action__MoveAction_Result * msg)
{
  if (!msg) {
    return false;
  }
  // final_coordinate
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_Result__fini(bme_gazebo_sensors__action__MoveAction_Result * msg)
{
  if (!msg) {
    return;
  }
  // final_coordinate
}

bool
bme_gazebo_sensors__action__MoveAction_Result__are_equal(const bme_gazebo_sensors__action__MoveAction_Result * lhs, const bme_gazebo_sensors__action__MoveAction_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // final_coordinate
  if (lhs->final_coordinate != rhs->final_coordinate) {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_Result__copy(
  const bme_gazebo_sensors__action__MoveAction_Result * input,
  bme_gazebo_sensors__action__MoveAction_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // final_coordinate
  output->final_coordinate = input->final_coordinate;
  return true;
}

bme_gazebo_sensors__action__MoveAction_Result *
bme_gazebo_sensors__action__MoveAction_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Result * msg = (bme_gazebo_sensors__action__MoveAction_Result *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_Result));
  bool success = bme_gazebo_sensors__action__MoveAction_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_Result__destroy(bme_gazebo_sensors__action__MoveAction_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_Result__Sequence__init(bme_gazebo_sensors__action__MoveAction_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Result * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_Result *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_Result__Sequence__fini(bme_gazebo_sensors__action__MoveAction_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_Result__Sequence *
bme_gazebo_sensors__action__MoveAction_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Result__Sequence * array = (bme_gazebo_sensors__action__MoveAction_Result__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_Result__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_Result__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_Result__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_Result__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_Result__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_Result * data =
      (bme_gazebo_sensors__action__MoveAction_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
bme_gazebo_sensors__action__MoveAction_Feedback__init(bme_gazebo_sensors__action__MoveAction_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_coordinate
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_Feedback__fini(bme_gazebo_sensors__action__MoveAction_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_coordinate
}

bool
bme_gazebo_sensors__action__MoveAction_Feedback__are_equal(const bme_gazebo_sensors__action__MoveAction_Feedback * lhs, const bme_gazebo_sensors__action__MoveAction_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_coordinate
  if (lhs->current_coordinate != rhs->current_coordinate) {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_Feedback__copy(
  const bme_gazebo_sensors__action__MoveAction_Feedback * input,
  bme_gazebo_sensors__action__MoveAction_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_coordinate
  output->current_coordinate = input->current_coordinate;
  return true;
}

bme_gazebo_sensors__action__MoveAction_Feedback *
bme_gazebo_sensors__action__MoveAction_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Feedback * msg = (bme_gazebo_sensors__action__MoveAction_Feedback *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_Feedback));
  bool success = bme_gazebo_sensors__action__MoveAction_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_Feedback__destroy(bme_gazebo_sensors__action__MoveAction_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__init(bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Feedback * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_Feedback *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__fini(bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_Feedback__Sequence *
bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * array = (bme_gazebo_sensors__action__MoveAction_Feedback__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_Feedback__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_Feedback * data =
      (bme_gazebo_sensors__action__MoveAction_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "bme_gazebo_sensors/action/detail/move_action__functions.h"

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__init(bme_gazebo_sensors__action__MoveAction_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!bme_gazebo_sensors__action__MoveAction_Goal__init(&msg->goal)) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(bme_gazebo_sensors__action__MoveAction_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  bme_gazebo_sensors__action__MoveAction_Goal__fini(&msg->goal);
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__are_equal(const bme_gazebo_sensors__action__MoveAction_SendGoal_Request * lhs, const bme_gazebo_sensors__action__MoveAction_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!bme_gazebo_sensors__action__MoveAction_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__copy(
  const bme_gazebo_sensors__action__MoveAction_SendGoal_Request * input,
  bme_gazebo_sensors__action__MoveAction_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!bme_gazebo_sensors__action__MoveAction_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

bme_gazebo_sensors__action__MoveAction_SendGoal_Request *
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_SendGoal_Request * msg = (bme_gazebo_sensors__action__MoveAction_SendGoal_Request *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Request));
  bool success = bme_gazebo_sensors__action__MoveAction_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__destroy(bme_gazebo_sensors__action__MoveAction_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__init(bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_SendGoal_Request * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_SendGoal_Request *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__fini(bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence *
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * array = (bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_SendGoal_Request * data =
      (bme_gazebo_sensors__action__MoveAction_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__init(bme_gazebo_sensors__action__MoveAction_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__fini(bme_gazebo_sensors__action__MoveAction_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__are_equal(const bme_gazebo_sensors__action__MoveAction_SendGoal_Response * lhs, const bme_gazebo_sensors__action__MoveAction_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__copy(
  const bme_gazebo_sensors__action__MoveAction_SendGoal_Response * input,
  bme_gazebo_sensors__action__MoveAction_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

bme_gazebo_sensors__action__MoveAction_SendGoal_Response *
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_SendGoal_Response * msg = (bme_gazebo_sensors__action__MoveAction_SendGoal_Response *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Response));
  bool success = bme_gazebo_sensors__action__MoveAction_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__destroy(bme_gazebo_sensors__action__MoveAction_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__init(bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_SendGoal_Response * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_SendGoal_Response *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__fini(bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence *
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * array = (bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_SendGoal_Response * data =
      (bme_gazebo_sensors__action__MoveAction_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Request__init(bme_gazebo_sensors__action__MoveAction_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    bme_gazebo_sensors__action__MoveAction_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Request__fini(bme_gazebo_sensors__action__MoveAction_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Request__are_equal(const bme_gazebo_sensors__action__MoveAction_GetResult_Request * lhs, const bme_gazebo_sensors__action__MoveAction_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Request__copy(
  const bme_gazebo_sensors__action__MoveAction_GetResult_Request * input,
  bme_gazebo_sensors__action__MoveAction_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

bme_gazebo_sensors__action__MoveAction_GetResult_Request *
bme_gazebo_sensors__action__MoveAction_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_GetResult_Request * msg = (bme_gazebo_sensors__action__MoveAction_GetResult_Request *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Request));
  bool success = bme_gazebo_sensors__action__MoveAction_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Request__destroy(bme_gazebo_sensors__action__MoveAction_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__init(bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_GetResult_Request * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_GetResult_Request *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__fini(bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence *
bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * array = (bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_GetResult_Request * data =
      (bme_gazebo_sensors__action__MoveAction_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "bme_gazebo_sensors/action/detail/move_action__functions.h"

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Response__init(bme_gazebo_sensors__action__MoveAction_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!bme_gazebo_sensors__action__MoveAction_Result__init(&msg->result)) {
    bme_gazebo_sensors__action__MoveAction_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Response__fini(bme_gazebo_sensors__action__MoveAction_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  bme_gazebo_sensors__action__MoveAction_Result__fini(&msg->result);
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Response__are_equal(const bme_gazebo_sensors__action__MoveAction_GetResult_Response * lhs, const bme_gazebo_sensors__action__MoveAction_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!bme_gazebo_sensors__action__MoveAction_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Response__copy(
  const bme_gazebo_sensors__action__MoveAction_GetResult_Response * input,
  bme_gazebo_sensors__action__MoveAction_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!bme_gazebo_sensors__action__MoveAction_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

bme_gazebo_sensors__action__MoveAction_GetResult_Response *
bme_gazebo_sensors__action__MoveAction_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_GetResult_Response * msg = (bme_gazebo_sensors__action__MoveAction_GetResult_Response *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Response));
  bool success = bme_gazebo_sensors__action__MoveAction_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Response__destroy(bme_gazebo_sensors__action__MoveAction_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__init(bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_GetResult_Response * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_GetResult_Response *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__fini(bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence *
bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * array = (bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_GetResult_Response * data =
      (bme_gazebo_sensors__action__MoveAction_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "bme_gazebo_sensors/action/detail/move_action__functions.h"

bool
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__init(bme_gazebo_sensors__action__MoveAction_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!bme_gazebo_sensors__action__MoveAction_Feedback__init(&msg->feedback)) {
    bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(bme_gazebo_sensors__action__MoveAction_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  bme_gazebo_sensors__action__MoveAction_Feedback__fini(&msg->feedback);
}

bool
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__are_equal(const bme_gazebo_sensors__action__MoveAction_FeedbackMessage * lhs, const bme_gazebo_sensors__action__MoveAction_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!bme_gazebo_sensors__action__MoveAction_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__copy(
  const bme_gazebo_sensors__action__MoveAction_FeedbackMessage * input,
  bme_gazebo_sensors__action__MoveAction_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!bme_gazebo_sensors__action__MoveAction_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

bme_gazebo_sensors__action__MoveAction_FeedbackMessage *
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_FeedbackMessage * msg = (bme_gazebo_sensors__action__MoveAction_FeedbackMessage *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(bme_gazebo_sensors__action__MoveAction_FeedbackMessage));
  bool success = bme_gazebo_sensors__action__MoveAction_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__destroy(bme_gazebo_sensors__action__MoveAction_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__init(bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_FeedbackMessage * data = NULL;

  if (size) {
    data = (bme_gazebo_sensors__action__MoveAction_FeedbackMessage *)allocator.zero_allocate(size, sizeof(bme_gazebo_sensors__action__MoveAction_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = bme_gazebo_sensors__action__MoveAction_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__fini(bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence *
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * array = (bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence *)allocator.allocate(sizeof(bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__destroy(bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__are_equal(const bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * lhs, const bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence__copy(
  const bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * input,
  bme_gazebo_sensors__action__MoveAction_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(bme_gazebo_sensors__action__MoveAction_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    bme_gazebo_sensors__action__MoveAction_FeedbackMessage * data =
      (bme_gazebo_sensors__action__MoveAction_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!bme_gazebo_sensors__action__MoveAction_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          bme_gazebo_sensors__action__MoveAction_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!bme_gazebo_sensors__action__MoveAction_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
